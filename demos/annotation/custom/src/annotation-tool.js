function Annotator(options) {
    ...
    function cleanData(data) {
        ...
        for(var i=0; i<data.annotations.length; i++) {
            ...
            */
            ...
            if (data.annotations[i].entity.properties) {
    ...
    function loadAnnotations(key, analysis) {
        ...
        if (analysis == undefined) analysis = _options.analysis;

        jQuery.ajax({
            type:"POST",
            url:_options.url+"/"+_options.version+"/analysis/"+analysis+"/enhance?key="+key+"&out=json",
    ...
    function changeConfidence(key, analysis, value) {
        jQuery("#rle_more_or_less").hide();
        jQuery("#rle_right_inner_left_inner").empty();
        loadAnnotations(key, analysis);
    ...
    function annotate(key, analysis) {
    ...
    jQuery(_options.container).load(_options.template,function(val){
        ...
        jQuery("#rle_button_annotate").click(function(){
            if (valid_key) {
                var analysis = jQuery("select#analyses").find(":selected").val();
                annotate(key, analysis);
            } else {
                alert("you must provide a valid api key!");
            }
